...
func NewServiceController(s service.CategoryUseCase) *categoryController {
...
func (controller *categoryController) FetchControllerBySlug(w http.ResponseWriter, r *http.Request) {
	const perm = "category:read"
	allowed, err := model.CheckPermission(perm, r)
	if !allowed {
		log.Println(err)
